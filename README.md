...
- **The gate stays mechanical.** `04-verify` is never a workflow and never an agent — it's
  `cargo test` + `cargo clippy -D warnings`, plus three optional deepeners (`hex-lint`,
  `effect-audit`, `cargo-mutants`) that report a loud SKIP when missing, never a silent pass.
...
`da-state`, the run-state authority, on first use. The target project must be a git repository
with a clean working tree — the driver refuses a dirty one rather than mixing your uncommitted
work into the run.
...
freezes a `metrics.json` naming out-of-band edits, steer open-times, and reason codes — the
trial measures instead of self-grading.
