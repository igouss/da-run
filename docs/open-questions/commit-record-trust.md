...
**Status:** RESOLVED in da-run2 (2026-07-18) as option B: `bin/run record-commit` resolves the branch tip in the worktree's own git (refusing when the tip never moved or does not sit on base) and writes the `commit-verified` marker; `fs_snapshot` derives Committed from the marker alone. The agent's sha never enters the record.
